...
void DumpResourceEntry(PIMAGE_RESOURCE_DIRECTORY_ENTRY resDirEntry, DWORD64 resourceBase, DWORD level) {
    ...
    if (resDirEntry->OffsetToData & IMAGE_RESOURCE_DATA_IS_DIRECTORY) {
        DumpResourceDirectory((PIMAGE_RESOURCE_DIRECTORY)((DWORD64)(resDirEntry->OffsetToData & 0x7FFFFFFF) + (DWORD64)resourceBase),resourceBase, level, resDirEntry->Name);
        return;
    }
    ...
    pResDataEntry = (PIMAGE_RESOURCE_DATA_ENTRY)(DWORD64)(resourceBase + resDirEntry->OffsetToData);
...
void DumpResourceDirectory(PIMAGE_RESOURCE_DIRECTORY resDir, DWORD64 resourceBase, DWORD level, DWORD resourceType) {
    ...
    if (level == 1 && (resourceType == (DWORD64)RT_STRING))
    {
    ...
    // If it's a stringtable, save off info for future use
    if (level == 1 && (resourceType == (DWORD64)RT_DIALOG))
    {
...
DWORD GetOffsetToDataFromResEntry(DWORD64 base, DWORD64 resourceBase, PIMAGE_RESOURCE_DIRECTORY_ENTRY pResEntry) {
    ...
    pStupidResDir = (PIMAGE_RESOURCE_DIRECTORY)(DWORD64)(resourceBase + pResEntry->OffsetToDirectory);

    PIMAGE_RESOURCE_DIRECTORY_ENTRY pResDirEntry =
        (PIMAGE_RESOURCE_DIRECTORY_ENTRY)(pStupidResDir + 1);// PTR MATH

    PIMAGE_RESOURCE_DATA_ENTRY pResDataEntry = (PIMAGE_RESOURCE_DATA_ENTRY)(DWORD64)(resourceBase + pResDirEntry->OffsetToData);
...
void DumpDialogs(DWORD64 base, PIMAGE_NT_HEADERS pNTHeader, DWORD64 resourceBase, PIMAGE_RESOURCE_DIRECTORY_ENTRY pDlgResEntry, DWORD cDlgResEntries) {
	for (unsigned i = 0; i < cDlgResEntries; i++, pDlgResEntry++)
	{
		...
		if (HIWORD(*pDlgStyle) != 0xFFFF)
		{
			...
			{
				// Control item header....
				pDlgItemTemplate = (DLGITEMTEMPLATE*)(DWORD64)(((DWORD64)pDlgItemTemplate + 3) & ~3);
				...
				PBYTE pCreationData = (PBYTE)(((DWORD64)pTitle + 1) & 0xFFFFFFFE);
...
void DumpResourceSection(void* base, PIMAGE_NT_HEADERS pNTHeader) {
    ...
    DWORD64 resourcesRVA = GetImgDirEntryRVA(pNTHeader, IMAGE_DIRECTORY_ENTRY_RESOURCE);
    if (!resourcesRVA) return;
    resDir = (PIMAGE_RESOURCE_DIRECTORY) GetPtrFromRVA(resourcesRVA, pNTHeader, (DWORD64)base);
    if (!resDir) return;
    printf("Resources (RVA: %llX)\n", (DWORD64)resourcesRVA);
    DumpResourceDirectory(resDir, (DWORD64)resDir, 0, 0);
    printf("\n");
    if (!fShowResources) return;
    if (cStrResEntries)
    {
        printf("String Table\n");

        DumpStringTable((DWORD64)base, pNTHeader, (DWORD64)resDir, pStrResEntries, cStrResEntries);
        printf("\n");
    }
    ...
    if (cDlgResEntries)
    {
        ...
        DumpDialogs((DWORD64)base, pNTHeader, (DWORD64)resDir, pDlgResEntries, cDlgResEntries);
        printf("\n");
    }
}
