...
void VolumeIO::open(TCHAR *VolumeName) {
    ...
    if (VolumeHandle == INVALID_HANDLE_VALUE) {
        exitWithLastError((TCHAR *)_T("Failed opening the volume '%s'\n"), VolumeName);
    }
}
...
BOOL VolumeIO::read(LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead) {
    return ReadFile(VolumeHandle, lpBuffer, nNumberOfBytesToRead, lpNumberOfBytesRead, (LPOVERLAPPED) NULL);
}
